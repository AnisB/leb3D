...
namespace d3d12
{
	...
	namespace compute_shader
	{
		ComputeShader create_compute_shader(GraphicsDevice graphicsDevice, const ComputeShaderDescriptor& csd, bool experimental)
		{
			...
			if (csd.codeType == ShaderCodeType::Code)
			{
			...
			{
				IDxcBlob* blob = LoadDXILShader(filename);
			...
			cS->samplerHeaps.push_back(create_descriptor_heap_sampler(deviceI, max(samplerCount, 1)));
